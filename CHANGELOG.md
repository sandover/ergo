...
- `ergo show <epic>` now nests headings from each child task body beneath the
  generated task heading. Standalone task output and `show <id> --body` remain
  unchanged.
...
- The Go module path now includes its required `/v4` major-version suffix, so
  `go install github.com/sandover/ergo/v4/cmd/ergo@latest` resolves current
  releases instead of stopping at v1.2.0.
...
- Clarified `--help` and `quickstart` for agents starting with no prior context.
- Added safer guidance for sending multiline bodies through stdin.

### Fixed

- Corrected the completion example to show claim cleanup in `updated_fields`.
