...
func runBulkCreate(dir string, opts GlobalOptions, epicTitle string, epicBody string, tasks []EpicTaskInput, draft bool) (bulkCreateOutput, error) {
	...
	if _, err := repository.UpdateWithJournal(func(graph *Graph) ([]Event, []JournalEntry, error) {
		events, journal, planned, err := planBulkCreate(graph, epicTitle, epicBody, tasks, draft)
		if err != nil {
			return nil, nil, err
		}
		out = planned
...
func planBulkCreate(graph *Graph, epicTitle string, epicBody string, tasks []EpicTaskInput, draft bool) ([]Event, []JournalEntry, bulkCreateOutput, error) {
	working := graph
	...
	if err != nil {
		return nil, nil, bulkCreateOutput{}, err
	}
	...
	for _, taskInput := range tasks {
		...
		for _, dep := range taskInput.After {
			...
			if hasCycle(working, fromID, toID) {
				return nil, nil, bulkCreateOutput{}, errors.New("dependency would create a cycle")
			}
	...
	return newEvents, journal, out, nil
}
