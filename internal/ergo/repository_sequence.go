...
func writeLinkEvents(dir string, opts GlobalOptions, eventType string, edges []sequenceEdge) ([]sequenceEdge, error) {
	...
	_, err := repository.Update(func(graph *Graph) ([]Event, error) {
		events, planned, err := planLinkEvents(graph, eventType, edges)
		if err != nil {
			return nil, err
		}
		changed = planned
...
func planLinkEvents(graph *Graph, eventType string, edges []sequenceEdge) ([]Event, []sequenceEdge, error) {
	working := graph
	...
	for _, edge := range edges {
		...
		if _, ok := working.Tombstones[from]; ok {
			return nil, nil, prunedErr(from)
		}
		if _, ok := graph.Tombstones[to]; ok {
			return nil, nil, prunedErr(to)
		}
		fromItem, ok := working.Tasks[from]
		if !ok {
			return nil, nil, fmt.Errorf("unknown id %s", from)
		}
		toItem, ok := working.Tasks[to]
		if !ok {
			return nil, nil, fmt.Errorf("unknown id %s", to)
		}
